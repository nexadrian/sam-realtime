...
class EfficientSamOnnxSegmenter(Segmenter):
    ...
    def segment(
        ...
        def run_model():
            (embeddings,) = self.encoder.run(None, {"batched_images": prepared.input_tensor})
            output_masks, iou_predictions, _low_res = self.decoder.run(
                None,
                {
                    "image_embeddings": embeddings,
                    "batched_point_coords": np.array([[[model_point]]], dtype=np.float32),
                    "batched_point_labels": np.array([[[1]]], dtype=np.float32),
                    "orig_im_size": np.array([1024, 1024], dtype=np.int64),
                },
            )
            return output_masks, iou_predictions

        (output_masks, iou_predictions), inference_ms = timed_call(run_model)
        masks = sigmoid(np.asarray(output_masks[0, 0], dtype=np.float32))
        scores = np.asarray(iou_predictions[0, 0], dtype=np.float32)
